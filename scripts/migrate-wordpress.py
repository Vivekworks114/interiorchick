#!/usr/bin/env python3
"""Migrate WordPress WXR export into Astro content collections for interiorchick.nl."""
...
XML_PATH = Path(
    os.environ.get(
        "WXR_PATH",
        "/Users/gauravkashyap/Downloads/interiorchicknl.WordPress.2026-06-27.xml",
    )
)
...
def prepare_mdx_body(html: str) -> str:
    ...
    try:
        ...
        # MDX treats <url> autolinks as JSX — convert to markdown links
    ...
    if slugs_path.exists():
        for slug in json.loads(slugs_path.read_text(encoding="utf-8")):
            ...
            products[slug] = {
                "slug": slug,
                "title": f"Beste {title}",
                "description": f"Lees alles over {title.lower()} op InteriorChick.nl.",
...
def backup_product_pages() -> dict[str, tuple[str, str | None]]:
    """Preserve beste-* pages and their HTML bodies."""
    backed_up: dict[str, tuple[str, str | None]] = {}
    if not PAGES_DIR.exists():
        return backed_up
    for path in PAGES_DIR.glob("beste-*.mdx"):
        html_path = HTML_DIR / f"{path.stem}.html"
        html = html_path.read_text(encoding="utf-8") if html_path.exists() else None
        backed_up[path.stem] = (path.read_text(encoding="utf-8"), html)
    return backed_up


def restore_product_pages(backed_up: dict[str, tuple[str, str | None]]) -> None:
    for slug, (mdx_content, html) in backed_up.items():
        (PAGES_DIR / f"{slug}.mdx").write_text(mdx_content, encoding="utf-8")
        if html:
...
def migrate() -> None:
    ...
    product_backup = backup_product_pages()

    for directory in (BLOG_DIR,):
        if directory.exists():
    ...
    if PAGES_DIR.exists():
        for f in PAGES_DIR.glob("*.mdx"):
            if not f.stem.startswith("beste-"):
                f.unlink()
    ...
    for item in pages_raw:
        ...
        HTML_DIR.mkdir(parents=True, exist_ok=True)
        (HTML_DIR / f"{item['slug']}.html").write_text(body, encoding="utf-8")
    ...
    restore_product_pages(product_backup)
